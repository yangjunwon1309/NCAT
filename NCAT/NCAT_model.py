...
class NCAT():
    def __init__(self, X = np.array([]) , y= np.array([]) , cluster_k = 7, obj = "regression", max_iter = 50000, N_bins = 10):
        self.X = X
        self.y = y
    ...
    def data_generator(self, scale_and_split = True):
        ...
        self.ncat = NCAT_datageneration(X_train, y_train, iter = 1500, depth = 2, lr=0.05, k= self.cluster_k, obj = self.obj, N_bins = self.N_bins)
    ...
    def distillation(self, lr = 0.07):
        ...
        for _ in range(self.cluster_k):
            ...
            if rank == 2:
                hidden_dim_list = [8, 4]
                max_iter = max(int((7/self.cluster_k)*((data.shape[1] * 10) + 20))*100, default_iter)
                tolerance_list = [0.95, y_rmse/3]
            elif rank == 1 : 
                hidden_dim_list = [4, 4]
                max_iter = max(int((7/self.cluster_k)*((data.shape[1] * 15) + 15))*100, default_iter)
                tolerance_list = [0.9, y_rmse/3]
            else :
                hidden_dim_list = [4, 4]
                max_iter = max(int((7/self.cluster_k)*((data.shape[1] * 12.5) + 15))*100, default_iter)
            ...
            self.NN_dict[_] = DNN_LM(input_dim= data.shape[1], hidden_dim=hidden_dim_list[0], hidden_dim_2=hidden_dim_list[1], output_dim= label.shape[1])
            adam_train(self.NN_dict[_], data, label, batch_size= batch_size_, tolerance_r2= tolerance_list[0], tolerance_rmse = tolerance_list[1], max_iterations=max_iter, lr= lr, beta1=0.9, beta2=0.999, epsilon=1e-8, weight_decay=0.0, loss_type = loss_type)
    ...
    def save(self, file_name = "model_info"):
        ...
        model_info["data_info"] = {"obj": self.obj, "cluster_k": self.cluster_k,
                                   "max_iter": self.max_iter, "catboost_dir": self.catboost_dir} if (self.obj == "regression") else {"obj": self.obj, "cluster_k": self.cluster_k, 
    ...
    def load(self, file_name = "model_info"):
        self.NN_dict = {}
        ...
        for nn_idx in [int(i) for i in data.keys() if i not in ["whole_param", "bias_and_scale", "scaler", "data_info"]]:
            dimension = data[str(nn_idx)]["dimension"]
            param = data[str(nn_idx)]["param"]
            features = data[str(nn_idx)]["features"]
            self.NN_dict[nn_idx] = DNN_LM(input_dim= dimension[0], hidden_dim= dimension[1], hidden_dim_2= dimension[2], output_dim= dimension[3])
...
def online_finetune(NCAT, data=[], label=[], max_iter = 100, batch_size = 16, lr = 0.01,):
    ...
    adam_train(NCAT, data, label, batch_size= batch_size, tolerance_r2= tolerance_list[0], tolerance_rmse = tolerance_list[1], max_iterations=max_iter, lr= lr, beta1=0.9, beta2=0.999, epsilon=1e-8, weight_decay=0.0, loss_type=loss_type)
