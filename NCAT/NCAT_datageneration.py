...
class NCAT_datageneration():
    ...
    def boundary(self, n_bins=10):
        ...
        conti_bounds = np.array(
            [self.X[continuous_columns].min(axis=0).values, self.X[continuous_columns].max(axis=0).values])
    ...
    def Grid_Search_CATBoost(self):
        ...
        CAT_list = [[iterations, depth, learning] for iterations in CAT_params["iterations"] for depth in
                    CAT_params["depth"] for learning in CAT_params["learning"]]
        list_ = CAT_list
            ...
            for _ in range(n_repeats):
                ...
                for train_index, test_index in kf.split(pd.concat([X_data, Y_data], axis=1)):
                    ...
                    if self.obj == "regression":
                        model = CatBoostRegressor(iterations=model_param[0], depth=model_param[1],
                                                  learning_rate=model_param[2], loss_function='RMSE')
                    elif self.obj == "classfication":
                        model = CatBoostClassifier(iterations=model_param[0], depth=model_param[1],
                                                   learning_rate=model_param[2])
    ...
    def make_catboost(self, grid_search=True):
        ...
        if self.obj == "regression":
            self.catboost = CatBoostRegressor(iterations=self.iter, depth=self.depth, learning_rate=self.lr,
                                              loss_function='RMSE')
        elif self.obj == "classfication":
    ...
    def DataGeneration_cluster(self, j=0, J_sub=0.6, N_data=1000, sampling="gibbs"):
        ...
        if sampling == "gibbs":
            ...
            gibbs_data = np.array(pd.concat([self.X.iloc[:, jth_features]] + gibbs_data_list, axis=0)).reshape(-1,
                                                                                                               len(jth_features))
            ...
            x_samples = np.random.uniform(self.bounds[0][jth_features], self.bounds[1][jth_features],
                                          size=(N_data, len(jth_features)))

            if random_leaf:
                batch = self.X.shape[0]
                for _ in range(N_data // batch + 1):
                    x_samples_ = x_samples[_ * batch:min((_ + 1) * batch, N_data)].copy()
                    selected_indices = np.random.choice(jth_cluster.index, size=int(J_sub * len(jth_cluster)),
                                                        replace=False, )
                    for index in selected_indices:
                        x_samples_ = self.DataGeneration_DT(index, len(x_samples_), -1, x_samples_,
                                                            jth_features=jth_features)
                ...
                for _ in range(self.N_bins):
                    ...
                    selected_indices = np.random.choice(jth_cluster.index, size=int(J_sub * len(jth_cluster)),
                                                        replace=False, )
                    for index in selected_indices:
                        x_samples_ = self.DataGeneration_DT(index, len(x_samples_), _, x_samples_, leaf_bins,
                                                            jth_features)
                    x_samples[_ * batch:min((_ + 1) * batch, N_data)] = x_samples_
    ...
    def DataGeneration_DT(self, tree_idx, n_samples=1, j=0, x_samples=[], leaf_bins=np.array([]), jth_features=[]):
        ...
        integer_idxs = [jth_features.index(col) for col in [idx for idx, c_col in enumerate(self.X.columns) if
                                                            (("D" in c_col) or ("B" in c_col)) and (
                                                                        idx in used_features)]]
        onehot_idxs = [jth_features.index(col) for col in
                       [idx for idx, c_col in enumerate(self.X.columns) if ("O" in c_col) and (idx in used_features)]]
            ...
            for direction in binary:
                ...
                if direction == 0:
                    if x_samples[_][x_samples_idx] > split_conditions[n][1]:
                        low, high = self.bounds[0][feature_to_split], max(self.bounds[0][feature_to_split],
                                                                          split_conditions[n][1])
                    ...
                    if x_samples[_][x_samples_idx] <= split_conditions[n][1]:
                        ...
                        low, high = min(self.bounds[1][feature_to_split], split_conditions[n][1]), self.bounds[1][
                            feature_to_split]
                        x_samples[_][x_samples_idx] = np.random.uniform(low, high)
